...
export default function FeatureCard({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
      {imagePosition === "left" && (
        <div className={`flex justify-center ${isIcon ? "md:justify-start" : ""}`}>
          <div className={isIcon ? "w-32 h-32" : "w-full max-w-md"}>
            <Image
              ...
              className={`${isIcon ? "object-contain" : "object-cover rounded-lg"} w-full h-auto transition-transform duration-300 hover:scale-105`}
            />
          </div>
        </div>
      )}
